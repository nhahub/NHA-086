      ...
      <form method="post" enctype="multipart/form-data" action="/predict" class="row g-3">
        ...
        <div class="col-md-3">
          <select name="model" class="form-select">
            ...
            <option value="kmeans">KMeans (unsupervised)</option>
          </select>
        </div>
        <div class="col-md-3">
...
def plot_band_histograms(arr, max_bands=3):
    bands = arr.shape[0]
...
def predict():
    ...
    try:
        if model_choice == "rf":
            ...
            result = {"method": "RandomForest", "predicted_class": str(pred), "confidence": float(proba)}
        else:
            cluster = kmeans.predict(feats_scaled)[0]
            mapped = cluster_map.get(int(cluster), "Unknown")
            result = {"method": "KMeans+map", "cluster": int(cluster), "mapped_class": mapped}
    except Exception as e:
    ...
    try:
        ...
        img_hist = plot_band_histograms(arr, max_bands=3)
